...
TEMPLATE="run_NDCalibration_single.sh"
...
INFOPATH="/storage/xenon/${USER}/NDCalibration/${DIRNAME}/info.txt"

# Make data storage directories
mkdir -p "/storage/xenon/$USER/NDCalibration/$DIRNAME/raw"
mkdir -p "/storage/xenon/$USER/NDCalibration/$DIRNAME/proc"
mkdir -p "/storage/xenon/$USER/NDCalibration/$DIRNAME/log"
...
for ((i = 1; i <= END_INDEX; i++)); do
    ...
    JOBSCRIPT="$TMPDIR/NDCalibrationsub_${JOBID}.sh"
